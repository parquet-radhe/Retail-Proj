...
def get_customers_schema():
    schema = "customer_id int, customer_fname string, customer_lname string, username string, password string, address string, state string, city string, pincode string"
    return schema

# creating customers dataframe
def read_customers(spark,env):
    conf = ConfigReader.get_app_config(env)
    customers_file_path = conf['customer.file.path']
    return spark.read\
        .format("csv")\
        .option("header","true")\
